...
export default function Plant(props) {
    ...
    const getHistory = () => {
        axios.get(`/api/plants/${props.plant._id}/history`, { headers })
            .then(res => setHistoryArray(res.data))
            .catch(err => console.log('Error | ', err))
    }
    const getNotes = () => {
        if (props.plant !== undefined) {
            axios.get(`/api/plants/${props.plant._id}/notes`, { headers })
    ...
    const displayPlant = (props) => {
        ...
        if (plant) {
            return (
                <div>
                    {(!sendingFeedback && !movingPlant) &&
                        (
                            <div className='plant singlePlant'>
                                {/* {console.log('health: ', plant.health.water)} */}
                                ...
                                <div className="locationInfo">
                                    ...
                                    <div id="plantInfo">
                                        ...
                                        <p>Need water every <span>{plant.health.water.days_between}</span> day</p>
                                        <p className="fert">Need fertilizer every <span>{plant.health.fertilizer.days_between}</span> day</p>
                                    ...
                                    {history ? history.map(history => {
                                        return (
                                            <div className="historyCard" key={history._id}>
                                                <div>
                                                    <p>WIP</p>
                                                    <p>{history.type.charAt(0).toUpperCase() + history.type.slice(1)}</p>
                                                    <p>{history.note}</p>
                                                </div>
                                                <div>
                                                    <p>{ChangeTime(history.date)}</p>
                                                    {history.user_id ? (
                                                        <p>{history.user_id.name.first} {history.user_id.name.last}</p>
                                                    ) : (
                                                        <p>Unknown</p>
                                                    )}
                                                </div>
                                ...
                                */}
                                    ...
                                    <div className="taskbtns">
                                        <div id="taskL">
                                            <button disabled={waterDue >= 2 ? true : false} className="wtrbtn">Watered</button>

                                            <button disabled={fertDue >= 2 ? true : false} className="fertbtn">Fertalized</button>
                                        </div>

                                        <div id="taskR">
                                            <button onClick={() => setMovingPlant(true)} className="mvebtn">Move Plant</button>

                                            <button onClick={() => setSendingFeedback(true)} className="fbbtn">Feedback</button>
                                        </div>

                                    </div>
                                </div>
                                <button onClick={() => console.log('Edit')} className="viewbtn">Edit Plant</button>
                                <button onClick={() => props.onClick()} className="viewbtn">Back</button>
                            </div >
                        )}
                    { sendingFeedback && (
                        <div>
                            <NewFeedback onClick={() => setSendingFeedback(false)} />
                        </div>
                    )}
                    { movingPlant && (
                        <div>
                            <MovePlant onClick={() => setMovingPlant(false)} locations={props.locations} buildings={props.buildings} />
                        </div>
                    )}
                </div >
            )
