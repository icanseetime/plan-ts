    ...
    if (wTasks.length > 0) {
        wTasksList = wTasks.map((task) => (
            <div className="oneTask" key={task._id}>
                ...
                <p>
                    {calcDaysRemaining(task.health.water.due) + 1} day
                    {calcDaysRemaining(task.health.water.due) + 1 !== 1
                        ? 's'
                        : ''}{' '}
                    remaining.
                </p>
    ...
    if (fTasks.length > 0) {
        fTasksList = fTasks.map((task) => (
            <div className="oneTask" key={task._id}>
                <h3 className="taskName">{task.name}</h3>
                <p>
                    {calcDaysRemaining(task.health.fertilizer.due) + 1} day
                    {calcDaysRemaining(task.health.fertilizer.due) + 1 !== 1
                        ? 's'
                        : ''}{' '}
                    remaining.
                </p>
            ...
            <div className="tasksContainer">
                <div>
                    ...
                    {wTasksList ? (
                        ...
                        <p className="dueTime">
                            No watering tasks in the near future
                        </p>
                    )}
                </div>
                <div>
                    ...
                    {fTasksList ? (
                        ...
                        <p className="dueTime">
                            No fertilizing tasks in the near future
                        </p>
                    )}
                </div>
            </div>
